...
class User:
    ...
    @classmethod
    def save(cls,data):
        query = "INSERT INTO test_user (first_name,last_name,email,age,password) VALUES (%(first_name)s,%(last_name)s,%(email)s,%(age)s,%(password)s);"
        some_id = connectToMySQL(DATABASE).query_db(query,data)
        return some_id
    
    @classmethod
    def get_one_by_email(cls, data:dict) -> object:
        query = "SELECT * FROM test_user WHERE email = %(email)s"
    ...
    def validator(form_data):
        ...
        if int(form_data['age']) < 13:
            is_valid = False
            flash("sorry kiddo...", "err_age")
